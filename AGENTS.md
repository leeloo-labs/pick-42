...
- Lane and matching build names use the set's themed archetype names — HOB: Boros Dwarves, Rakdos Amass, Golgari Ferocious, Azorius Recruit, Simic Elves; SOS, where the college name stands alone without a guild word: Silverquill (Repartee), Prismari (Opus, Elementals, spell-only mana, cast-a-spell-this-turn gates), Lorehold (cards leaving your graveyard, flashback), Quandrix (Increment, Fractals, X costs), Witherbloom (Infusion, lifegain triggers, Pests); the corpus color map recognizes the college names — only when the draft is in that set and the pool contains at least two cards showing the tribe or mechanic; otherwise the plain guild name stands. Themes never cross sets: each `LANE_THEMES` entry in the blend engine carries its set code, a set without entries shows plain guild names (a Dwarf in SOS is not a Boros Dwarves signal), and naming a new set's archetypes means new entries under its own code. A stored review whose deck name carries another set's theme is repaired to its guild name when reviews load.
...
- Keep ranking behavior inspectable and add regression tests for any recommendation the user identifies as clearly wrong. `npm run replay:check` compares curated decision scenarios with a reviewed baseline and reports ranks, scores, reasons, and pair changes. Update the baseline only after reviewing an intentional change; diagnostic cases are not evidence of predictive accuracy.
