...
contextBridge.exposeInMainWorld('draftCompanion', {
  ...
  exportBackup: (recipes) => ipcRenderer.invoke('draft:export-backup', recipes),
  previewBackup: (text, recipes) => ipcRenderer.invoke('draft:preview-backup', text, recipes),
  restoreBackup: (token, recipes) => ipcRenderer.invoke('draft:restore-backup', token, recipes),
